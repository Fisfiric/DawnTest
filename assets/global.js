...
const elements = document.querySelectorAll('div[quick-add="true"]');

elements.forEach((element) => {
  element.addEventListener('mouseenter', () => {
    const overlay = element.querySelector('.variant-picker-overlay');
    overlay.classList.add('variant-picker-overlay--active');
  });

  element.addEventListener('mouseleave', () => {
    const overlay = element.querySelector('.variant-picker-overlay');
    overlay.classList.remove('variant-picker-overlay--active');
  });
});
...
class VariantGroup extends HTMLElement {
  constructor() {
    ...
    this.addEventListener('click', (event) => {
      if (event.target.classList.contains('variant-button')) {
        event.preventDefault();
        this.handleVariantButtonClick(event.target);
      }
    });
  }

  handleVariantButtonClick(button) {
  ...
  toggleButton(button) {
    ...
    // this.variantButtons.forEach(function (button) {
    //   button.addEventListener('click', function () {
    //     this.productId = button.getAttribute('data-product-id');
    //     this.productButtons = this.querySelectorAll(
    //       '.variant-button[data-product-id="' + this.productId + '"]'
    //     );
...
document.addEventListener('DOMContentLoaded', function () {
  var continueButtons = document.querySelectorAll('[id^="enable_variants"]');

  continueButtons.forEach(function (button) {
    button.addEventListener('click', function () {
      var productId = this.id.replace('enable_variants', '');
      var variantPicker = document.getElementById('variant-picker' + productId);
      if (variantPicker) {
...
class VariantSubmit extends HTMLElement {
  constructor() {
    super();
    this.observer = new MutationObserver(this.checkSelection.bind(this));
  }

  connectedCallback() {
    this.variantGroups = this.querySelectorAll('variant-group');
    this.variantGroups.forEach((group) => {
      this.observer.observe(group, { attributes: true });
    });
    this.checkSelection();
  }

  checkSelection() {
    const allSelected = Array.from(this.variantGroups).every(
      (group) => group.getAttribute('data-isSelected') === 'true'
    );
    if (allSelected) {
      this.getVariantJSON();
      this.getSelectedOptions();
      this.getSelectedVariant();
      this.IDinput = this.parentElement.querySelector('.product-variant-id');
      this.IDinput.setAttribute('value', this.currentVariant.id);
      this.addToCart();
      //this.submitForm();
      //this.ajaxSubmit();
      //this.add_to_cart_grid(this.currentVariant.id);
    }
  }
  ajaxSubmit() {
    this.form = this.closest('form');
  ...
  fakeSubmit() {
    console.log(
      'All variant-group elements are selected. Simulating form submission.'
    );
    // Implement your fake submit logic here
    // For example, dispatching a custom event
    this.dispatchEvent(
      new CustomEvent('formSubmit', { bubbles: true, cancelable: true })
    );
  }
  ...
  getSelectedOptions() {
    this.options = Array.from(
      this.querySelectorAll('.variant-button.active'),
  ...
  addToCart() {
    ...
    this.lastActiveInputId = this.currentVariant.id;
  ...
  toggleButtonsOff() {
    const buttons = this.querySelectorAll('.variant-button.active');
    buttons.forEach((button) => {
      button.classList.remove('active');
    });
  }
...
customElements.define('variant-submit', VariantSubmit);
